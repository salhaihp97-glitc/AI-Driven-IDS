...
class FlowFeatureCalculator:
    ...
    def __init__(self, activity_timeout_seconds: float = 5.0) -> None:
        """
        Initializes the statistical feature extractor.

        Args:
            activity_timeout_seconds: Gap (in seconds) between consecutive packets
                that closes the current active burst and opens an idle period. Mirrors
                the CICFlowMeter convention used to generate the CICIDS2017 training
                corpus and is environment-driven in the live capture pipeline.
        """
        self._activity_timeout_seconds: Final[float] = activity_timeout_seconds
    ...
    def _active_idle_stats(self, timestamps: np.ndarray) -> tuple[float, float, float, float, float, float, float, float]:
        """
        ...
        corpus: a gap larger than the configured activity timeout (5 seconds by default,
        derived empirically from the dataset's Idle/Flow-IAT boundary) closes the current
        active burst and opens an idle period equal to that gap. Durations are in
        microseconds.
        ...
        activity_timeout_us: Final[float] = self._activity_timeout_seconds * 1_000_000.0
